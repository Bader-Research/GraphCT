...
int64_t graphio_b(graph *G, char *filename, int64_t mode, struct graph_shared ** shared, char ** name) {
  int64_t i;
  int64_t NE, NV;
  size_t sz;
  ...
  if (mode == 4)
  {
    ...
    if (shared == NULL)
      ...
      alloc_shared_graph (G, NV, NE, shared, name);
...
int64_t fwrite_doubles(double *input, int64_t num, char *filename)
{
  ...
  char *output = xmalloc ( (num * 24+1) * sizeof(char));
...
{
  char *output = xmalloc ( (num * 11+1) * sizeof(char));
