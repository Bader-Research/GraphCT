...
{ 
	...
	uint32_t *output = xmalloc (arraySize * sizeof(uint32_t));
...
MTA("mta assert nodep")
	...
	int64_t arraySize = 3 + NV + NE + NE;
	uint64_t *output = xmalloc (arraySize * sizeof(uint64_t));

	output[0] = (uint64_t) NE;
	output[1] = (uint64_t) NV;
	output[2] = 0;

MTA("mta assert nodep")
	for (i = 0; i < NV; i++)
	{
		output[3 + i] = (uint64_t) G->edgeStart[i];
	}

MTA("mta assert nodep")
	for (i = 0; i < NE; i++)
	{
		output[3 + NV + i] = (uint64_t) G->endVertex[i];
	}

MTA("mta assert nodep")
	for (i = 0; i < NE; i++)
	{
		output[3 + NV + NE + i] = (uint64_t) G->intWeight[i];
	}

	printf("\nWriting graph data to disk...\n");
	fflush(stdout);
	xmt_luc_snapout (filename, output, arraySize*sizeof(uint64_t));
	printf("done.\n");
