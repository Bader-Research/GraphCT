...
{ 
  ...
  if (options.graph_type == 1)
  {
    ...
    graphio_b(G, options.infilename, 4, NULL, NULL);
    G->undirected = 1;
  }
  else if (options.graph_type == 5)
  {
    G = (graph *) xmalloc(sizeof(graph));
    graphio_b(G, options.infilename, 8, NULL, NULL);
    G->undirected = 1;
  }
  ...
  else if (options.graph_type == 3)
  {
    ...
    computeGraph (&Gdirected, &SDGdata, NULL, NULL);
    G = makeUndirected (&Gdirected, NULL, NULL);
    free_graph (&Gdirected);
    free_edgelist (&SDGdata);
  }
  else if (options.graph_type == 6)
  {
    G = (graph *) xmalloc (sizeof(graph));
    struct graphs_shared * shared;
    graph_shared_map (G, &shared, options.infilename);
  }
  else {
  ...
  if (options.kernel_id == 12) {
    ...
    printf("\ncentrality()%s beginning execution...\n",
	(exact? " EXACT" : ""));
    fflush (stdout);

    stats_tic ("centrality");
    timeBC = centrality(G, BC, Vs);
    stats_toc ();

    if (options.outfilename)
      fwrite_doubles(BC, G->numVertices, options.outfilename);
    printf("\tcentrality() completed execution.\n");
  ...
  if (options.kernel_id == 10) {
    printf("\nRunning Bellman-Ford...\n");
    fflush (stdout);
    int64_t source = options.parameter1;
    int64_t checksum;
    int64_t * dist = xmalloc( (G->numVertices+1) * sizeof(*dist));
    time = timer();
    stats_tic ("bellmanford");
    bellman_ford(G, dist, source, &checksum);
    stats_toc ();
    time = timer() - time;
    printf("Time taken is %9.6lf sec.\n", time);
    if (options.outfilename)
      fwrite_ints(dist, G->numVertices, options.outfilename);
    free(dist);
  }
  ...
  if (options.kernel_id == 11) {
    printf("\nRunning Delta Stepping...\n");
    fflush (stdout);
    int64_t source = options.parameter1;
    double delta = options.parameter4;
    double * dist = xmalloc( (G->numVertices+1) * sizeof(*dist));
    time = timer();
    stats_tic ("deltastepping");
    delta_stepping(G, dist, source, &delta);
    stats_toc ();
    time = timer() - time;
    printf("Time taken is %9.6lf sec.\n", time);
    if (options.outfilename)
      fwrite_doubles(dist, G->numVertices, options.outfilename);
    free(dist);
  }
  ...
  if (options.kernel_id == 13) {
    printf("\nRunning PageRank...\n");
    fflush (stdout);
    double * rank = xmalloc( G->numVertices * sizeof(double));
    time = timer();
    stats_tic ("pagerank");
    pagerank(G, rank, 0.000001, 0.85, 30);
    stats_toc ();
    time = timer() - time;
    printf("Time taken is %9.6lf sec.\n", time);
    free(rank);
